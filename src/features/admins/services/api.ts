...
const adminsApiService = {
  ...
  getRolesPermissionDetail: async (id: string) => {
    const response = await appAxios.get<APIResponse<RoleData>>(
      `role-and-permission/roles/${id}`
    );
    return response.data;
  },
  createRolesPermissions: async (data: CreateRoleRequest) => {
    const response = await appAxios.post<APIResponse<RoleData>>(
      "role-and-permission/roles",
      data
    );
    return response.data;
  },
  ...
  getAdmins: async (filter: AdminFilter) => {
    const params = objectToQueryString(filter);
    const response = await appAxios.get<APIResponse<AdminData[]>>(
      `auth/admin/all?${params}`
    );
    return response.data;
  },
  ...
  createAdmin: async (data: CreateAdminRequest) => {
    ...
    const response = await appAxios.post<APIResponse<AdminData>>(
      "auth/admin/create",
      rest
    );
    return response.data;
  ...
  updateAdmin: async (data: UpdateAdminRequest) => {
    ...
    const response = await appAxios.patch<APIResponse<AdminData>>(
      `auth/admin/${id}`,
      rest
    );
    return response.data;
