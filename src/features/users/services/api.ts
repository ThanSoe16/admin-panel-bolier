...
const userApiService = {
  getUsers: async (filter: UserFilter) => {
    ...
    const response = await appAxios.get<APIResponse<UserData[]>>(
      `/users/users-list?${params}`
    );
    return response.data;
  },
  getUserDetail: async (id: string) => {
    const response = await appAxios.get<APISResponse<UserDetailData>>(
  ...
  getUserPurchasedTemplates: async (filter: UserFilter) => {
    ...
    const response = await appAxios.get<
      APIResponse<UserPurchaseTemplatesData[]>
