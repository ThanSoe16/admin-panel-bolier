...
export const useGetHostingFeeIncomeReport = (filter: HostingFeeFilter) => {
  return useQuery({
    ...
    queryFn: () => hostingFeeIncomeReportApiService.getHostingFeeIncomeReport(filter),
    refetchOnWindowFocus: false,
  });
};

export const useGetHostingFeeIncomeMonthlyReport = (filter: HostingFeeFilter) => {
  return useQuery({
    queryKey: ['hosting-income-monthly-report', filter],
    queryFn: () => hostingFeeIncomeReportApiService.getHostingFeeIncomeMonthlyReport(filter),
    refetchOnWindowFocus: false,
  });
};
