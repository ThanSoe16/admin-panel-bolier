...
export const useGetTemplateIncomeReport = (filter: TemplateIncomeFilter) => {
  return useQuery({
    ...
    queryFn: () =>
      templateIncomeReportApiService.getTemplateIncomeReport(filter),
    refetchOnWindowFocus: false,
  });
};

export const useGetTemplateIncomeMonthlyReport = (
  filter: TemplateIncomeFilter
) => {
  return useQuery({
    queryKey: ["template-income-monthly-report", filter],
    queryFn: () =>
      templateIncomeReportApiService.getTemplateIncomeMonthlyReport(filter),
    refetchOnWindowFocus: false,
  });
};
