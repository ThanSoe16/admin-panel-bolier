...
export const updateContactUsSchema = contactSchema
  .pick({
    office: true,
    email: true,
    phoneNumbers: true,
  ...
  .extend({
    office: z
      .string({ message: 'Office is required' })
      .min(3, { message: 'Office must be at least 3 characters' })
      .max(255, { message: 'Office must be at most 255 characters' }),
    email: z.string({ message: 'Email is required' }).email({ message: 'Invalid Email Format' }),
    phoneNumbers: z
      .string({ message: 'Phone Number is required' })
      .min(3, { message: 'Phone Number must be at least 8 characters' }),
    operationHours: z
      .string({ message: 'Operation Hours is required' })
      .min(3, { message: 'Operation Hours must be at least 3 characters' })
      .max(255, { message: 'Operation Hours must be at most 255 characters' }),
    contents: z.array(updateContentSchema),
  });

export type UpdateContactUSType = z.infer<typeof updateContactUsSchema>;

export const updateContactApiSchema = updateContactUsSchema
  .omit({
    phoneNumbers: true,
  })
  .extend({
    phoneNumbers: z.array(
      z.string().min(1, { message: 'Phone Number must be at least 8 characters' }),
    ),
  });
