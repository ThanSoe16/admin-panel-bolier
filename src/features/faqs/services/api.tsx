...
import { CreateFAQRequest, FAQData, FAQFilterData, UpdateFAQRequest } from '../types';
import { objectToQueryString } from '@/utils/objectToQueryString';

const faqsApiService = {
  getFaqs: async (filters: FAQFilterData) => {
    const params = objectToQueryString(filters);
    const response = await appAxios.get<APIResponse<FAQData[]>>(`/admin-settings/faq?${params}`);
    return response.data;
  },

  getFaqById: async (id: string) => {
    const response = await appAxios.get<APIResponse<FAQData>>(`/admin-settings/faq/${id}`);
    return response.data;
  },
