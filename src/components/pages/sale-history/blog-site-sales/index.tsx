...
import { useGetBlogSiteSaleHistory } from "@/features/sale-histoy/blog-site/services/queries";
import dayjs from "dayjs";

const BlogSiteSales = () => {
  const { query, start, setStart, end, setEnd, word } = usePagination();
  const { data, isLoading } = useGetBlogSiteSaleHistory({
  ...
  return (
    <div>
      <PageTitle> Blog Site Sales </PageTitle>
        ...
        <DataTable
          ...
          renderHeader={() => (
            <div className="w-full flex flex-col md:flex-row flex-grow items-start md:items-center justify-between gap-2">
              <SearchInput placeholder="Search by username or Blog Name" />
              <DatePickerWithRange
                value={
                  start
                    ? {
