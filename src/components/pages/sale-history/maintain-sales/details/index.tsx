...
import { useGetMaintainSaleHistoryDetails } from "@/features/sale-histoy/maintain/services/queries";
...
const Details = () => {
  const { id } = useParams();
  const { data, isLoading } = useGetMaintainSaleHistoryDetails({
    id: id as string,
  });
  ...
  const feeDetails = [
      ...
      value: (
        <div className="flex items-center justify-center">
          <Status
            ...
            status={
              data?.body?.data?.MerchantBlogBillingStatus?.toLowerCase() ?? ""
            }
          />
        </div>
      ),
    ...
    {
      label: "Paid on",
      value: <p>{formatDate(data?.body?.data?.paidDate ?? "")}</p>,
    },
    {
      label: "Total",
      value: <p>${CurrencyFormat(data?.body?.data?.total ?? 0)}</p>,
    },
  ];
  ...
  return (
    <div className="space-y-6 mb-10">
      ...
      <DetailTable title="Maintain Fee Details" data={feeDetails} />
