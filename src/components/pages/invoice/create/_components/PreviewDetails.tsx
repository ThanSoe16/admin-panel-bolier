...
import { CreateInvoiceRequest } from "@/features/invoice/types";
...
const Details: React.FC<Props> = ({ data }) => {
  ...
  const billPayablePeriodEnd = dayjs(data.invoiceFor)
    .endOf("month")
    .add(15, "day");

  const invoiceCreatedAt = invoiceUserDetail?.data?.body?.data
    ?.latestServerFeeDate
    ? data?.invoiceFor
    : invoiceUserDetail?.data?.body?.data?.blogCreatedAt;
    ...
    <div className="p-4 md:p-12 bg-muted rounded-2xl">
      ...
      <div className="border-t border-b py-4 border-[#E0E0E0]">
        <p className="text-lg">
          ...
          {dayjs(invoiceCreatedAt).format("MMMM DD")} -{" "}
          {dayjs(data?.invoiceFor).endOf("month").format("MMMM DD, YYYY")}.
        </p>
        <p>
          {" "}
          Bill Payable Period: {today.format("DD MMM YYYY")} -{" "}
          {billPayablePeriodEnd.format("DD MMM YYYY")}.{" "}
        </p>
      </div>
