...
import { useUpdateMembershipPlan } from "@/features/membership/plans/services/mutations";
...
const UpdateMembership: React.FC<CreateModalProps> = ({
  open,
  handleClose,
  data,
}) => {
  const updatePlan = useUpdateMembershipPlan();
  const defaultValues: UpdateMembershipPlanRequest = {
    id: data.id,
    amount: data.amount,
    DurationType: data.DurationType,
  };
  const form = useForm<UpdateMembershipPlanRequest>({
    resolver: zodResolver(updateMembershipPlanSchema),
    defaultValues: defaultValues,
  });

  const submit = async (data: UpdateMembershipPlanRequest) => {
    updatePlan.mutateAsync(data).then((res) => handleClose());
  };
