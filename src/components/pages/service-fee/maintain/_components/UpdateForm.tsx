...
import { useCreateMaintainFee } from "@/features/service-fee/services/mutations";
...
const UpdateForm: React.FC<UpdateFormProps> = ({ fee = "0" }) => {
  ...
  const links = [
    { label: "Maintain Fee", href: "/service-fee/maintain" },
  ...
  const createMaintainFee = useCreateMaintainFee();

  const createHandler = () => {
    createMaintainFee.mutateAsync(value.toString()).then(() => router.back());
  };
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <div className="flex flex-row gap-4 justify-start md:justify-end items-center w-full lg:flex-1">
          ...
          <Button
            loading={createMaintainFee.isPending}
            addDoneIcon
