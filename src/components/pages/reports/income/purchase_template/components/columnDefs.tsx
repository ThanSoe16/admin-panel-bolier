...
const YearlyAction = (props: PurchasedTemplateData) => {
  return (
    <Link
      href={`/reports/income/blog_template_sale/monthly?date=${props.month}`}
    >
      <TableBaseButton uiType="details">Details</TableBaseButton>
    </Link>
  );
  ...
  [
    {
      ...
      size: 100,
      cell: ({ row }) => (
        <p className="">{row?.original?.Template?.templateCode}</p>
      ),
    },
    {
    ...
    },
    {
      ...
      cell: ({ row }) => {
        const contents =
          row?.original?.Template?.TemplateCategory?.TemplateCategoryContent ??
          [];
    ...
    },
    {
      ...
      header: "Total Amount",
      cell: ({ row }) => (
        <p className="">${CurrencyFormat(row?.original?.price)}</p>
      ),
    },
    {
...
export const purchaseTemplateMonthlyColumnDefs: ColumnDef<PurchasedTemplateData>[] =
  [
    ...
    },
    {
      ...
      size: 150,
      cell: ({ row }) => (
        <p className="">${CurrencyFormat(row?.original?.amount)}</p>
      ),
    },
    {
      accessorKey: "actions",
...
export const purchaseTemplateYearlyColumnDefs: ColumnDef<PurchasedTemplateData>[] =
  [
    ...
    },
    {
      ...
      size: 150,
      cell: ({ row }) => (
        <p className="">{CurrencyFormat(row?.original?.count)}</p>
      ),
    },
    {
      accessorKey: "actions",
...
export const purchaseTemplateCategoryColumnDefs: ColumnDef<TemplateSaleData>[] =
  [
    ...
    },
    {
      ...
      header: "Amount",
      cell: ({ row }) => (
        <p className="">${CurrencyFormat(row?.original?.totalPrice)}</p>
      ),
    },
    {
