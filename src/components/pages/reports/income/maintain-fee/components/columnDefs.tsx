...
const MonthlyAction = (props: MaintainFeeListData) => {
  return (
    <Link href={`/reports/income/blog_maintenance_fee/daily?date=${props.date}`}>
      <TableBaseButton uiType="details">Details</TableBaseButton>
    </Link>
  );
};
...
const YearlyAction = (props: MaintainFeeListData) => {
  return (
    <Link href={`/reports/income/blog_maintenance_fee/monthly?date=${props.month}`}>
      <TableBaseButton uiType="details">Details</TableBaseButton>
    </Link>
  );
};
...
export const maintainFeeDailyColumnDefs: ColumnDef<MaintainFeeListData>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => <p className="capitalize">{row?.original?.paidDate ? 'Paid' : 'Unpaid'}</p>,
  },
  {
    accessorKey: 'actions',
    header: 'Action',
