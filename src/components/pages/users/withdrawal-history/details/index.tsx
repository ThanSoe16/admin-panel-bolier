...
const WithdrawalHistoryDetail = ({ id }: { id: string }) => {
  ...
  const { data: adminData } = useGetAdminDetail(
    detailData?.ApprovedByAdminId ?? ""
  );
      ...
      value: (
        <Flex align={"center"} className="gap-1">
          ...
          <p className="capitalize">
            {detailData?.OnesiteWithdrawStatus.toLowerCase()}
          </p>
        </Flex>
      ),
    },
    ...
    },
    {
      label:
        detailData?.OnesiteWithdrawStatus === "SUCCESSFUL"
          ? "Confirmed on"
          : "Rejected on",
      value: detailData?.confirmedDate
        ? formatDate(detailData?.confirmedDate)
        : "",
    },
    {
      label:
