...
const DomainDetails = ({ id, domainId }: { id: string; domainId: string }) => {
  ...
  const domainDetails = [
    ...
    {
      label: "Expire on",
      value: data?.body?.data?.expiredAt
        ? formatDate(data?.body?.data?.expiredAt)
        : "-",
    },
    {
      label: "Register on",
      value: data?.body?.data?.registeredAt
        ? formatDate(data?.body?.data?.registeredAt)
        : "-",
    },
    {
      ...
      value: (
        <div className="flex items-center justify-center gap-2">
          ...
          <p className="capitalize">
            {data?.body?.data?.paymentStatus?.toLocaleLowerCase()}
          </p>
        </div>
      ),
    },
