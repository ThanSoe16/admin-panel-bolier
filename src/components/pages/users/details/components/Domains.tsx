...
const columnDefs: ColumnDef<UserDomainData>[] = [
  ...
  },
  {
    ...
    size: 150,
    header: 'Expire On',
    cell: ({ row }) => <span>{dayjs(row?.original?.expired_at).format('DD MMM YYYY')}</span>,
  },
  {
    accessorKey: 'status',
    size: 200,
    ...
    cell: ({ row }) => (
      <div className="flex flex-row items-center">
        <Status
          ...
          status={
            row?.original?.domain_accept === 'UNAVAILABLE'
              ? `In Use (${row?.original?.domain_connect_to_project})`
              : 'AVAILABLE'
          }
        />
      </div>
  ...
  },
  {
    ...
    size: 150,
    header: 'Purchased On',
    cell: ({ row }) => <span>{dayjs(row?.original?.updatedAt).format('DD MMM YYYY')}</span>,
  },
];
