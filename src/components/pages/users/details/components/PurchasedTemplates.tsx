...
const PurchasedTemplates = ({ id }: { id: string }) => {
  ...
  const defaultLanguageId = languageData?.body?.data?.find((lang) => lang.key === 'en')?.id || '';
          ...
          >
            ...
            <div className="flex flex-col text-sm gap-2 w-full">
              ...
              <div className="flex flex-row items-center ">
                <p className="text-default-secondary w-24 lg:w-[200px]">Category : </p>
              ...
              <div className="flex flex-row items-center ">
                <p className="text-default-secondary w-24 lg:w-[200px]"> Price : </p>
                <p className="font-bold"> {item.price === 0 ? 'Free(0$)' : item.price + '$'} </p>
              </div>
              <div className="flex flex-row items-center ">
                <p className="text-default-secondary w-28 lg:w-[200px]"> Purchased on : </p>
                <p className=""> {dayjs(item.createdAt).format('DD MMM YYYY, HH:mm')} </p>
              </div>
            </div>
          </div>
        ))
