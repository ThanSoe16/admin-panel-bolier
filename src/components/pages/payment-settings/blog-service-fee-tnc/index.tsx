...
const BlogServiceFeeTNC = () => {
  ...
  return (
    <div>
      <PageTitle> Blog Service Fee T&C </PageTitle>
      {isLoading ? (
        <Loading />
      ) : (
        data?.body?.data && (
          <Grid columns={{ initial: "1" }} className="gap-4">
            ...
            <TermsCard
              ...
              onEdit={() =>
                router.push(
                  `/payment-settings/tnc-blog-service-fee/detail?language=${engLanguage?.key}`
                )
              }
            />
          </Grid>
...
export default BlogServiceFeeTNC;
